#!/bin/bash
# CONTINUATION of the 2026-08-30 campaign: takes over after the original
# process is retired at the slide adaptive/K5wall boundary (its in-memory
# task_ladder lacked K5wall's step overrides). Shares CAMPAIGN_LOG_DIR with
# the original so the wall-clock budgets read the same campaign.log.
#
# The CENIC campaign, encoded: per task, in order —
#   K1..K5, adaptive, then K5wall (fixed K5 rerun at the SAME WALL
#   CLOCK the task's adaptive run actually consumed).
# Tasks in Marco's order: slide, lift, plate, flip, tree.
#
# NOT LAUNCHED BY ANYTHING AUTOMATICALLY. Launch only after the original
# campaign process is dead and no trainer owns the GPU.
#
...
k5wall() { # k5wall <gym-task> <short-name>
  ...
  # minus its summed iteration time. Runs at K5's step: the rung is "the
  # finest guess, wall-matched", so it must inherit K5's overrides.
  local gym=$1 t=$2
  ...
  run "$gym" icf "icf-fixed-$t-K5wall-s42" "cq_${t}_K5wall.log" "$budget" $K5
}
...
k5wall      IsaacContrib-Slide-Mug-Trossen-v0     slide
