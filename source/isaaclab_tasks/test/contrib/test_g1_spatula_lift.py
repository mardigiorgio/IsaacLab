...
def test_spatula_settles_on_tabletop(physics_preset_name):
    ...
    with _settled_env(physics_preset_name, steps=60) as env:
        ...
        # Distance is taken from the NOMINAL spawn, so the bound has to absorb
        # whatever reset randomization the task's EventCfg applies to the
        # spatula plus settle slop; a bound tighter than that envelope fails on
        # an undisturbed spatula.
