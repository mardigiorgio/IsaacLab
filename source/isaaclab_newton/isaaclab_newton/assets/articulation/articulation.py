...
class Articulation(BaseArticulation):
    ...
    def write_data_to_sim(self):
        ...
        if getattr(self, "_has_newton_actuators", False):
            ...
            # Rate-limit the commanded position target to each joint's configured velocity
            # limit before it is forwarded below: the clamp rewrites the user-order
            # ``_joint_pos_target_sim`` buffer in place, which both ordering branches read.
            if self._has_implicit_actuators and self._velocity_limit_clamp_active:
