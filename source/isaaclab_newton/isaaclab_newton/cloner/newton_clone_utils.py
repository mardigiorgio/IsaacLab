...
def build_source_builders(
    ...
    *,
    ...
    Args:
        ...
        simplify_meshes_exclude: Regex patterns matched against shape labels (prim
            paths): matching mesh shapes keep their RAW triangle geometry when
            ``simplify_meshes`` is enabled. Shapes carrying SDF/hydroelastic state
            are always kept raw -- hulling them would discard that state.
...
def _build_source_builder(
    ...
    """Build one source builder; an empty ``authored`` map hulls every hullable mesh.

    Only shapes reported by :func:`_hullable_shape_indices` are ever hulled: mesh shapes
    matching a ``simplify_meshes_exclude`` pattern or carrying SDF/hydroelastic state keep
    their raw triangle geometry. Authored ``physics:approximation`` modes are explicit USD
    intent and are applied regardless of the exclusion patterns.
    """
    ...
    if authored:
        ...
        if simplify_meshes:
            hullable = set(_hullable_shape_indices(builder, simplify_meshes_exclude))
            indices = [
                index
                for index in _unauthored_collision_mesh_shapes(builder, authored_shape_indices)
                if index in hullable
            ]
            if indices:
